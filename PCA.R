...
# LMIC run pca
dev.off()
pca.low = prcomp(na.omit(lowdat.log[,10:20]),center=TRUE,scale=TRUE)
...
# export the loadings
library(xlsx)
write.xlsx(pca.low$rotation, "pca.low.xlsx")

# read in PC1 and PC2 loadings
pca.xy = read.csv("pca.low.csv")

## making color-coded plot
library(ggplot2)
p =  ggplot(pca.xy, aes(x=PC1, y=PC2)) + geom_point(aes(colour=group), size=1)+
...
# HIC run pca
pca.high = prcomp(na.omit(highdat.log[,c(10:12,14:20)]),center=TRUE,scale=TRUE)
...
# export the loadings
library(xlsx)
write.xlsx(pca.high$rotation, "pca.high.xlsx")

# read in PC1 and PC2 loadings
pca.xy.high = read.csv("pca.high.csv")

## making color-coded plot
library(ggplot2)
ph =  ggplot(pca.xy.high, aes(x=PC1, y=PC2)) + geom_point(aes(colour=group), size=1, )+
  geom_text(aes(label=variable),hjust=.65, vjust=-1, size = 2)+
